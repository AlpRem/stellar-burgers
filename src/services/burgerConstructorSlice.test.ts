...
describe('Проверяют редьюсер слайса для конструктора бургера', () => {
  ...
  it('Тест добавления ингредиентов ', () => {
    ...
    expect(newIngredients.ingredients[0]).toEqual({
      ...mockIngredients[0],
      id: expect.any(String)
    });
    expect(newIngredients.ingredients[1]).toEqual({
      ...mockIngredients[1],
      id: expect.any(String)
    });
  });
  ...
  it('Тест удаления ингредиента ', () => {
    const newIngredients = burgerConstructorReducer(
      initialState,
      deleteIngredient(mockIngredients[0].id)
    );
  ...
  it('Тест изменения порядка ингредиентов ', () => {
    let newIngredients = burgerConstructorReducer(
      initialState,
      moveIngredient({ currentIndex: 0, newIndex: 1 })
    );
    ...
    newIngredients = burgerConstructorReducer(
      initialState,
      moveIngredient({ currentIndex: 1, newIndex: 1 })
    );
  ...
  it('Тест заполнения данных в конструктор ингредиентов ', () => {
    const newIngredients = burgerConstructorReducer(
      initialState,
      saveConstructorToStore(initialState)
    );
